# Akamai Application Security template, specifically created for Akamai App & API Protection (AAP) using the Adaptive Security Engine in Automatic mode (ASE_AUTO).
# This TF configuration uses best practices to utilize AAP and ASE to their best capabilities.
